...
def solve(data: str, days: int = 80) -> int:
    ...
    age_counts = [0 for _ in range(9)]  # ages can be 0 through 8
    ...
    while True:
        for age, count in enumerate(age_counts):
            if count != 0:
                ...
                # Simulate the oldest fishes aging to 0,
                # and then one more day, to 6. We do this by
                # left shifting the whole array by `age+1`.
                shift = age + 1
                age_counts[:-shift] = age_counts[shift:]

                # Also, set last `shift` values to zero
                age_counts[-shift:] = [0] * shift

                # Finally, add as many 6-age and 8-age fishes back.
                age_counts[6] += count
                age_counts[8] += count
