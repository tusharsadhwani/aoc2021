...
def parse_data(data: str) -> tuple[set[Dot], list[Fold]]:
    dot_locations, fold_lines = data.split("\n\n")

    dots: set[Dot] = set()
    for location in dot_locations.splitlines():
        x, y = location.split(",")
        dots.add((int(x), int(y)))

    folds: list[Fold] = []
    for fold_line in fold_lines.splitlines():
        _, fold_data = fold_line.rsplit(" ", 1)
        fold_direction, fold_position = fold_data.split("=")
        folds.append((fold_direction, int(fold_position)))

    return dots, folds


def get_new_position(dot: Dot, fold_direction: str, fold_position: int) -> Dot:
    """Returns new position of a dot, after folding"""
    x, y = dot
    if fold_direction == "x":
        return (min(x, 2 * fold_position - x), y)
    elif fold_direction == "y":
        return (x, min(y, 2 * fold_position - y))
    else:
        raise ValueError(f"Unknown fold direction: {fold_direction}")


def fold(dots: set[Dot], fold_direction: str, fold_position: int) -> set[Dot]:
    """Returns a set of dots created after executing a fold"""
    new_dots: set[Dot] = set()
    for dot in dots:
        new_position = get_new_position(dot, fold_direction, fold_position)
        new_dots.add(new_position)
...
def part1(data: str) -> int:
    ...
    first_fold = folds[0]
    fold_direction, fold_position = first_fold

    overlapped_dots = fold(dots, fold_direction, fold_position)
